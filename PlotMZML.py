...
with h5py.File(time_mz_HDF5_file_directory, "r") as hf1, h5py.File(time_intensity_HDF5_file_directory, "r") as hf2:
    ...
    TimeArray = np.array(hf1.get('time'))
    nScans = len(TimeArray)
    TicArray = np.array(hf1.get('tic'))
    ...
    intesity_vs_mz_plot_dict['x'] = np.array(hf1.get(str(InitialTimePoint)))
    intesity_vs_mz_plot_dict['y'] = np.array(hf2.get(str(InitialTimePoint)))
